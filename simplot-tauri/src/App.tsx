...
function App() {
  ...
  const loadFile = useCallback(async (path: string) => {
    ...
    try {
      ...
      setLoadedFiles((current) => [...current, path]);
      ...
      <aside className="side-pane" aria-label="Plot controls">
        ...
        <section className="control-group series-group">
          ...
          <div className="series-list">
            {series.map((item) => (
              <label className="series-row" key={item.id}>
                <input
                  ...
                  onChange={(event) =>
                    updateSeries(item.id, { visible: event.target.checked })
                  }
                />
                <span
                  className="swatch"
                ...
                <input
                  ...
                  onChange={(event) =>
                    updateLabelDraft(item.id, event.target.value)
                  }
                  onKeyDown={(event) => {
                    if (event.key === "Enter") {
            ...
            {series.length === 0 && (
              <div className="empty-list">No series loaded</div>
            )}
          </div>
        </section>
